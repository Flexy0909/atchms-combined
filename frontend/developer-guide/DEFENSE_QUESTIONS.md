...
### Q13: How did you implement the frosted-glass look on the login page?
- **Answer**: "I used CSS **Glassmorphism**. By setting the container background to a translucent white (`rgba(255, 255, 255, 0.08)`) and using the CSS property `backdrop-filter: blur(25px) saturate(180%)`, the browser dynamically blurs and saturates whatever part of the building background image sits behind the login card, giving it a premium, modern feel."

### Q14: Why did you override the browser default dialogs (alert/confirm), and how does it work?
- **Answer**: "The standard browser `alert()` and `confirm()` dialogs look basic and unprofessional. I created custom overlay modals (`ATCHMS.alert` and `ATCHMS.confirm`) using dynamic DOM manipulation inside `api.js`. These modals inject a styled HTML structure, dim the screen with a blur filter, and use CSS transitions to slide the dialog box into view. They returns JS Promises so they can be awaited like standard prompts, allowing us to keep clean, asynchronous flow control."
