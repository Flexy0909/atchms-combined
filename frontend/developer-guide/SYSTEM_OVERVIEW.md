...
## 5. Modern UI/UX Architecture Features

The interface uses advanced CSS styling and helper scripts to deliver a premium experience:
- **Split-Pane Glassmorphic Login**: Implements a full-screen layout utilizing an actual ATC building background (`atc-building.jpg`) layered under a dark green overlay. The sign-in card utilizes modern **frosted glassmorphism** styling via `backdrop-filter: blur(25px) saturate(180%)` and subtle translucent borders.
- **Custom Themed Dialog Modals**: We replaced default native browser `alert()` and `confirm()` prompts with a unified custom modal popup engine injected dynamically by `api.js`. The popup dialogs auto-detect keywords to change theme accents (e.g., green for Success, red for Error, gold for Confirmations) complete with screen-dimming backdrop-blur overlays.
- **Micro-Animations & Dynamic Loaders**: Dynamic pages feature rotating circular SVG spinner components and pulse transitions during data fetches from the MySQL REST API, preventing jarring static jumps during loading.
